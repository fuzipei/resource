...
export async function database(){const url=variables().REDIS_URL;if(!url)throw new ApiError('数据库尚未配置',503);const db=createClient({url,socket:{connectTimeout:8000,reconnectStrategy:false}});await db.connect();return db}
