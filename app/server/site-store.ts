...
export function databaseUrl(){const value=(process.env.REDIS_URL||'').trim();if(!value)throw new ApiError('数据库未配置：请在 Vercel Production 环境设置 REDIS_URL 并重新部署',503);try{const u=new URL(value);if(!['redis:','rediss:'].includes(u.protocol)||!u.hostname)throw Error();if(u.protocol==='redis:'&&u.hostname.endsWith('.upstash.io'))return value.replace(/^redis:/i,'rediss:');}catch{throw new ApiError('REDIS_URL 格式不正确，请使用 Redis 连接串，不能填写 HTTPS REST 地址',503)}return value}
