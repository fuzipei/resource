...
async function handle(r:Request){
 ...
 // Stage bounded batches; publish once so retries cannot duplicate or expose partial playlists.
 if(action==='import-save'&&body.importId!==undefined){
  if(typeof body.importId!=='string'||!/^[-_a-zA-Z0-9]{12,80}$/.test(body.importId)||!Number.isSafeInteger(body.total)||body.total<1||!Number.isSafeInteger(body.offset)||body.offset<0||body.offset%50!==0||!Array.isArray(body.songs)||body.songs.length!==Math.min(50,body.total-body.offset)||body.offset>=body.total)throw new InputError('导入批次无效');
  const id='import_'+body.importId,stageKey=PREFIX+'import:'+uid+':'+body.importId+':';
  const batch=body.songs.map((s:any)=>{body.song=s;return song()});
  await db.set(stageKey+body.offset,JSON.stringify({total:body.total,songs:batch}),{EX:86400});
  if(body.offset+batch.length<body.total)return reply({savedCount:body.offset+batch.length});
  const existing=(await read(uid)).data;if(existing.playlists.some((p:any)=>p.id===id))return reply({library:existing,savedCount:body.total});
  const collected:any[]=[];
  for(let offset=0;offset<body.total;offset+=50){const raw=await db.get(stageKey+offset);if(!raw)throw new InputError('导入批次已过期或缺失，请重新保存',409);const chunk=JSON.parse(raw);if(chunk.total!==body.total||!Array.isArray(chunk.songs)||chunk.songs.length!==Math.min(50,body.total-offset))throw new InputError('导入批次不一致，请重新保存',409);collected.push(...chunk.songs)}
  body.songs=collected;
 }
 ...
 else if(action==='import-save'){if(body.importId&&data.playlists.some((p:any)=>p.id==='import_'+body.importId))return reply({library:data});if(data.playlists.length>=100)throw new InputError('最多创建 100 个歌单');if(!Array.isArray(body.songs)||!body.songs.length)throw new InputError('请至少导入一首歌曲');const songs=body.songs.map((s:any)=>{body.song=s;return song()});const name=String(body.name||'导入的歌单').trim().slice(0,60);data.playlists.push({id:body.importId?'import_'+body.importId:randomBytes(12).toString('hex'),name:name||'导入的歌单',songs,source:String(body.source||'外部歌单').slice(0,40)})}
