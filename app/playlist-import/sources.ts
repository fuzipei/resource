...
export async function readNeteaseTracks(ids:string[]):Promise<ExternalTrack[]>{if(!Array.isArray(ids)||!ids.length||ids.length>200||ids.some(id=>typeof id!=='string'||!/^\d{1,18}$/.test(id)))throw Error('歌曲资料批次无效');const byId=new Map((await catalogTracks(ids)).map(t=>[String(t.id),t]));return ids.flatMap(id=>byId.has(id)?[byId.get(id)]:[]).map(t=>({catalogId:'wy_'+t.id,title:t.name,artist:(t.ar||t.artists||[]).map((a:any)=>a.name).join(' / '),album:(t.al||t.album)?.name,durationMs:t.dt||t.duration}))}
export async function readExternal(input:string):Promise<ExternalPlaylist>{
 ...
 const host=u.hostname;let tracks:ExternalTrack[]=[],name='导入的歌单',source='',warning='';
 ...
 tracks=initial.map((t:any)=>({catalogId:'wy_'+t.id,title:t.name,artist:(t.ar||t.artists||[]).map((a:any)=>a.name).join(' / '),album:(t.al||t.album)?.name,durationMs:t.dt||t.duration}));
 ...
 const j=JSON.parse(await get('https://c.y.qq.com/qzone/fcg-bin/fcg_ucc_getcdinfo_byids_cp.fcg?type=1&json=1&utf8=1&onlysong=0&format=json&disstid='+id,{headers:{Referer:'https://y.qq.com/'}}));const p=j.cdlist?.[0];if(!p)throw Error('QQ 歌单暂时无法读取，请使用文件导入');name=p.dissname;source='QQ 音乐';tracks=(p.songlist||[]).map((t:any)=>({catalogId:t.songmid?'qq_'+t.songmid:undefined,title:t.songname||t.name,artist:(t.singer||[]).map((a:any)=>a.name).join(' / '),album:t.albumname||t.album?.name,durationMs:(t.interval||0)*1000}));if((p.total_song_num||p.songnum)>tracks.length)warning=`平台仅返回 ${tracks.length} / ${p.total_song_num} 首。`;
 }else if(['www.kugou.com','m.kugou.com'].includes(host)){
 const id=u.pathname.match(/(?:special\/single\/|plist\/index\/)(\d+)/)?.[1]||u.searchParams.get('specialid');if(!id||!/^\d+$/.test(id))throw Error('请使用酷狗 special/single/… 或 plist/index/… 完整歌单链接');
 const html=await get('https://www.kugou.com/yy/special/single/'+id+'.html');const rawSongs=html.match(/var\s+data\s*=\s*(\[[\s\S]*?\])\s*,\s*specialData\s*=/)?.[1];const rawInfo=html.match(/var\s+specialInfo\s*=\s*(\{[\s\S]*?\});/)?.[1];source='酷狗音乐';name=rawInfo?JSON.parse(rawInfo).name||name:name;tracks=(rawSongs?JSON.parse(rawSongs):[]).map((t:any)=>({title:t.songname||t.audio_name,artist:t.singername||t.author_name||(t.authors||[]).map((a:any)=>a.author_name).join(' / ')||'未知歌手',album:t.album_name,durationMs:t.timelength||t.duration}));warning='酷狗公开页面可能仅返回前 30 首，请核对数量。完整歌单建议使用导出文件。'; }else if(host==='open.spotify.com'){
 const id=u.pathname.match(/\/(?:intl-[^/]+\/)?playlist\/([A-Za-z0-9]+)/)?.[1];if(!id)throw Error('请使用 Spotify playlist 歌单链接');
 const html=await get('https://open.spotify.com/embed/playlist/'+id);const data=jsonAt(html,'id="__NEXT_DATA__"');const p=data?.props?.pageProps?.state?.data?.entity;if(!p)throw Error('Spotify 公开页面无法读取，请导出 CSV / JSON 后导入');source='Spotify';name=p.name||p.title||name;tracks=(p.trackList||[]).map((t:any)=>({title:t.title,artist:t.subtitle, durationMs:t.duration}));warning='Spotify 公开预览可能只提供部分曲目。请核对数量；完整歌单建议使用导出文件。';
 }else if(['music.youtube.com','www.youtube.com','youtube.com'].includes(host)){
 const id=u.searchParams.get('list');if(!id||!/^[-_a-zA-Z0-9]{8,150}$/.test(id))throw Error('请使用包含 list 参数的 YouTube Music 歌单链接');
